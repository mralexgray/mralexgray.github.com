...
/* jshint -W007, -W033, -W098, -W099, -W116, node */
...
// loader
(function (c, f) {
    Array.prototype.foreach = function (h) {
        for (var g = 0; g < this.length; g++) {
            h(g, this[g])
        }
    };
    asyncLoader = function (i, h) {
        i.foreach(function (k, j) {
            e(j, d(j), h)
        })
...
var foundation = [
    ...
    // mrgraycssjflatuipro/UI/bootstrap/css/bootstrap-responsive.css',
...
var flatui = function () {
    asyncLoader(['/flatui/UI/css/flat-ui.css'], { 'callback': function () {
        console.log('loaded /flatui/UI/css/flat-ui.css')
        asyncLoader(['/flatui/UI/js/', '/flatui/UI/js/flatui-radio.js'], {
            'callback': function () {
                console.log('loaded flat-ui js')
            }
        })
    }
    })
...
var flatlibs = [ 'jquery.ui.touch-punch.min.js',
...
Object.values = function (obj) {
    var vals = [];
    for (var key in obj) if (obj.hasOwnProperty(key)) vals.push(obj[key]);
    return vals;
}
window._console = { log: function () {
    _.each(arguments, function (arg) {
        log(stringify(arg, true));
    });
},
    dir: function () {
...
var libs = function () {
    return _.union(_.map(jqplugins, function (js) {
            return mrgrayjs + '/' + js;
        }),
        _.map(flatlibs, function (js) {
            return flatuiJS + '/' + js;
        }),
        otherlibs);
}
...
_docReady = function (more){
  console.log ('loaded mrgray.com JS')
  WebFontConfig = { google: { families: [ 'Ubuntu+Mono:400,700:latin' ] }    };
  doDocReady = function (more){

    if (_.isfunction (docReady)) {
      console.log ('loading docReady');
      docReady ();
    }
    if (_.isArray (docJS)) {
      asyncLoader (
        docJS, { 'callback': function (){
          console.log ('loaded docJS');
          doDocReady ();
        } }
      )
    }
...
// START!
  asyncLoader (
    foundation, {'callback': function (){
      console.log ('loaded jquery')
      asyncLoader (
        foundation2, {'callback': function (){
          console.log ('loaded jqueryUI + underscore.js')
          asyncLoader (
            libs (),
            {'callback': function (){
              _docReady ();
